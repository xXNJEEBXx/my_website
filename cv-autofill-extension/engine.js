...
window.__CV_APP.Engine = (function() {
  ...
  async function executeOracleOptionClick(el, value) {
    ...
    let targetItem = listItems.find(li => {
      const text = (li.innerText !== undefined ? li.innerText : li.textContent || '').trim().toLowerCase();
      return text === valStr || text.includes(valStr);
    });
    ...
    if (!targetItem) {
        ...
        const newListItems = Array.from(document.querySelectorAll('.oj-listbox-result, .oj-listbox-item, li[role="option"]'));
        targetItem = newListItems.find(li => {
            const text = (li.innerText !== undefined ? li.innerText : li.textContent || '').trim().toLowerCase();
            return text === valStr || text.includes(valStr);
        });
    }
